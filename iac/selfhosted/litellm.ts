...
/**
 * Deploys the LiteLLM inference proxy into the infrastructure namespace.
 * Owns everything LiteLLM-scoped: the DB-init job against shared-postgres, the
 * runtime config.yaml, the SelfhostedApp proxy app, the /metrics block route,
 * the namespace security baseline, and the ServiceMonitor + NetworkPolicies.
 *
 * LiteLLM deliberately runs WITHOUT Redis: a single pod / single uvicorn worker
 * means rate limits, budgets, router state, and cache invalidation all live in
 * one process, so the Redis model's cross-worker coordination adds no value.
 * The no-Redis banner is suppressed via LITELLM_DISABLE_NO_REDIS_WARNING.
 */
export function deployLitellm(args: DeployLitellmArgs) {
...
`
  ...
  // Deploy LiteLLM using standard self-hosted app module. This exposes the proxy publicly
  // with automatic Let's Encrypt TLS cert management via Traefik.
  const app = new SelfhostedApp("litellm", {
    ...
    env: [
      ...
      // No Redis: this is a single-pod / single-worker deployment, so LiteLLM's
      // per-process state (rate limits, budgets, router cache) needs no cross-worker
      // coordination. Suppress the banner that assumes a multi-worker Redis topology.
      { name: "LITELLM_DISABLE_NO_REDIS_WARNING", value: "true" },
    ],
    databases: [
      {
        type: "postgres",
